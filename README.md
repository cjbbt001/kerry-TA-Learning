...
| 顺序 | 目录 | 内容 |
| --- | --- | --- |
| 1 | `Assets/Unity_shader入门/` | ShaderLab/CG 基础：纹理采样、极坐标 UV、溶解、透明混合、Fresnel 边缘光 |
| 2 | `Assets/ASE入门/` | Amplify Shader Editor 练习：扫光与边缘光、MatCap 与薄膜干涉、顶点生长、Stencil |
| 3 | `Assets/lit shader/` | 自定义 Blinn-Phong 光照：贴图、法线、阴影、视差与多光源 |
| 4 | `Assets/environment-mapping/` | 环境映射：Cubemap、IBL、球谐与 Light Probe，以及玉石材质 |
| 5 | `Assets/MobileCharacterRendering/` | 轻量级角色渲染：皮肤近似、各向异性头发高光、SH 与 IBL |
...
对应资产：`Assets/Unity_shader入门/`、`Assets/ASE入门/`。
...
> `Assets/ASE入门` 不包含 Amplify Shader Editor 插件，需要从 Unity Asset Store 自行导入。

## 2. Blinn-Phong Lit Shader

从基础光照开始，逐步实现一个自定义 Built-in 管线材质 Shader，并在单体材质和小型场景中验证。

对应资产：`Assets/lit shader/`。
...
从 Cubemap 环境反射出发，逐步练习 IBL、Spherical Harmonics、Reflection Probe 与 Light Probe。

对应资产：`Assets/environment-mapping/`。
...
在环境映射基础上，组合基础漫反射、厚度透射、环境反射、Fresnel 和附加光源，在实时渲染预算内近似通透感。
